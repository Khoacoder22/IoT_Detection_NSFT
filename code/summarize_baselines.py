#!/usr/bin/env python3
"""Aggregate baselines_default per-run JSONs into summary.csv (same pattern as Scene B)."""
...
RUNS_DIR = RESULTS_DIR / "baselines_default" / "runs"
OUT_CSV = RESULTS_DIR / "baselines_default" / "summary.csv"

COLUMNS = [
    "dataset", "model", "seed",
    "n_train", "n_test", "n_classes",
    "MCC", "ACC",
    "TPR_macro", "TPR_weighted", "FPR",
...
def main() -> int:
    ...
    for p in sorted(RUNS_DIR.glob("*.json")):
        ...
        if d.get("_status") == "error" or "_error" in d:
            rows.append({
                "dataset": d.get("_dataset", ""), "model": d.get("_model", ""),
                "seed": c.get("seed", ""), "status": "error",
            })
            continue
        rows.append({
            "dataset": d["_dataset"], "model": d["_model"], "seed": c.get("seed", ""),
    ...
    print(f"[summary] wrote {len(rows)} rows → {OUT_CSV}  (ok={len(rows)-errs} err={errs})")
    return 0
